...
In the **Noisy regime**, the data is degraded but recoverable. Instead of sending the data directly to the model, the system first routes it to the Stabilization Layer. This layer attempts to improve the data using data-type-specific techniques (rolling median, interpolation, EWMA, etc.), after which the data is sent back to the DRS Layer to be re-scored. If the improvement succeeds, a final confidence score (Recovered) is calculated: $Recovered = Stabilized \times e^{-(1-Stabilized) \times IS}$. If the Recovered score exceeds the 0.80 threshold, the data moves into the Clean regime; if it falls within 0.25–0.79, it is routed directly to the Fallback Model (not sent back through the Stabilization Layer); if it drops below 0.25, the system moves to the Information Collapse regime.

In the **Corrupted regime**, the main model can no longer be trusted. In its place, a low-complexity, wide-confidence-interval, conservative Fallback Model takes over. This model does not make sharp, confident predictions; its purpose is simply to provide a reasonable and safe output. The rule for transitioning to Information Collapse differs by Fallback Model type: for regression tasks (Olist), a live, record-level Confidence Gate compares the bootstrap-CI-based Confidence Signal calculated for each prediction against a threshold ($\tau_{Fallback,regression}$), and any record falling below it is routed directly to Information Collapse. For classification tasks (UJIIndoorLoc, Yahoo Finance), if the model's validation accuracy falls below a threshold ($\tau_{Fallback,classification}$), Fallback routing for that domain is disabled once during IP2 calibration and is not recomputed live. RMSE/MAE never triggers this decision live — it is used offline only during IP2 to calibrate the empirical relationship between the Confidence Signal and actual RMSE.
