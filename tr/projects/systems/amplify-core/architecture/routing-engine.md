...
**Bozuk rejimde** artık ana modele güvenilmez. Onun yerine düşük karmaşıklıklı, geniş güven aralıklı, temkinli bir Yedek Model (Fallback Model) devreye girer. Bu model kesin ve iddialı tahminler yapmaz; amacı sadece makul ve güvenli bir çıktı vermektir. Bilgi Çöküşü'ne geçiş kuralı Yedek Model türüne göre farklılaşır: regresyon görevlerinde (Olist) her tahmin için hesaplanan bootstrap-CI tabanlı Güven Sinyali, kayıt bazlı canlı bir Güven Kapısı ile ($\tau_{Fallback,regresyon}$) karşılaştırılır ve eşiğin altında kalan kayıt doğrudan Bilgi Çöküşü'ne yönlendirilir. Sınıflandırma görevlerinde (UJIIndoorLoc, Yahoo Finance) ise modelin doğrulama accuracy'si $\tau_{Fallback,sınıflandırma}$ eşiğinin altında kalırsa, o domain için Yedek Model yönlendirmesi İP 2 kalibrasyonunda bir kez devre dışı bırakılır; canlı akışta tekrar hesaplanmaz. RMSE/MAE değeri bu kararı canlı olarak doğrudan tetiklemez; yalnızca İP 2'de Güven Sinyali ile gerçek RMSE arasındaki ampirik ilişkiyi kalibre etmek için çevrimdışı kullanılır.
